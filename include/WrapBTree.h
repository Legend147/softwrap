...
#define leafslotmax  16

    /// Base B+ tree parameter: The number of key slots in each inner node,
    /// this can differ from slots in each leaf.
#define innerslotmax  16

    /// Computed B+ tree parameter: The minimum number of key/data slots used
    /// in a leaf. If fewer slots are used, the leaf will be merged or slots
    /// shifted from it's siblings.
#define minleafslots  (leafslotmax / 2)

    /// Computed B+ tree parameter: The minimum number of key slots used
    /// in an inner node. If fewer slots are used, the inner node will be
    /// merged or slots shifted from it's siblings.
#define mininnerslots  (innerslotmax / 2)
...
template <typename _Key, typename _Data,
          ...
          typename _Alloc = 
		pstd::PersistentAllocator<_Value>
		//std::allocator<_Value> 
	>
class btree
{
public:
	void * operator new(size_t sz) {return pmalloc(sz);}
...
private:
	...
	/// Define an related allocator for the inner_node structs.
        ...
        WrapVar<key_type>        slotkey[innerslotmax];

        /// Pointers to children
        WrapVar<node*>          childid[innerslotmax+1];
	...
	/// Define an related allocator for the leaf_node structs.
        ...
        WrapVar<leaf_node *> prevleaf;

        /// Double linked list pointers to traverse the leaves
        WrapVar<leaf_node *> nextleaf;

        /// Keys of children or data pointers
        WrapVar<key_type>        slotkey[leafslotmax];

        /// Array of data
        WrapVar<data_type>       slotdata[leafslotmax];
...
public:
    ...
    struct tree_stats
    {
        ...
        WrapVar<size_type>       itemcount;

        /// Number of leaves in the B+ tree
        WrapVar<size_type>       leaves;

        /// Number of inner nodes in the B+ tree
        WrapVar<size_type>       innernodes;

        /// Base B+ tree parameter: The number of key/data slots in each leaf
        WrapVar<unsigned short >    leafslots;

        /// Base B+ tree parameter: The number of key slots in each inner node.
        WrapVarUS     innerslots ;
        ...
        {
        	leafslots = leafslotmax;
        	innerslots = innerslotmax;
        }
...
private:
    ...
    WrapVar<leaf_node *> headleaf;

    /// Pointer to last leaf in the double linked leaf chain
    WrapVar<leaf_node *> tailleaf;
...
private:
    ...
    */
    ...
    {
        leaf_node *n =  new (get_allocator().allocate(sizeof(leaf_node))) leaf_node();
        	//	new (leaf_node_allocator().allocate(1)) leaf_node();  //  ERG changed from leaf_node_allocator().
        //printf("allocate_leaf=%p\n", n);
        n->initialize();
        stats.leaves++;
        return n;
    }
    ...
    {
        inner_node *n = new (get_allocator().allocate(sizeof(inner_node))) inner_node();
...
public:
    ...
    inline iterator end()
    {
        return iterator(tailleaf, tailleaf ? (unsigned short)((node*)tailleaf)->slotuse : 0);
    }
...
private:
    ...
    inline int find_lower(const node_type *n, const key_type& key) const
    {
        ...
        int lo = 0;
        int hi = (int)((unsigned short)n->slotuse - 1);
...
private:
    ...
    struct node* copy_recursive(const node *n)
    {
        ...
        else
        {
            const inner_node *inner = static_cast<const inner_node*>(n);
            int l = inner->level;
            inner_node *newinner = allocate_inner(l);
...
public:
    ...
    //  Dummy function to insert dummy data.
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
        ...
        if (newchild)
        {
        	unsigned short s = ((node *)(root))->level;
        	short unsigned int i = s + 1;
            inner_node *newroot = allocate_inner(i);
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
        ...
        if (!n->isleafnode())
        {
            ...
            node *tn = inner->childid[slot];
            //printf("inner(%p)->childid[slot=%d] = %p\n", inner, slot, tn);
            std::pair<iterator, bool> r = insert_descend(tn,
        ...
        else // n->isleafnode() == true
        {
            ...
            while(i >= 0 && key_less(key, leaf->slotkey[i])) {
                leaf->slotkey[i + 1] = (key_type)(leaf->slotkey[i]);
                leaf->slotdata[i + 1] = (data_type)(leaf->slotdata[i]);
                i--;
            }
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        newleaf->nextleaf = (leaf_node*)(leaf->nextleaf);
        if (newleaf->nextleaf == NULL) {
            BTREE_ASSERT(leaf == tailleaf);
            tailleaf = newleaf;
        }
        else {
            ((leaf_node*)(newleaf->nextleaf))->prevleaf = newleaf;
        }

        for(unsigned int slot = mid; slot < (unsigned)leaf->slotuse; ++slot)
        {
            unsigned int ni = slot - mid;
            newleaf->slotkey[ni] = (key_type)(leaf->slotkey[slot]);
            newleaf->slotdata[ni] = (data_type)(leaf->slotdata[slot]);
        }
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        short unsigned int lev = inner->level + 0;
        inner_node *newinner = allocate_inner(lev);

        newinner->slotuse = inner->slotuse - (mid + 1);

        for(unsigned int slot = mid + 1; slot < (unsigned)inner->slotuse; ++slot)
        {
...
private:
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            {
                leaf->slotkey[i] = (key_type)(leaf->slotkey[i + 1]);
                leaf->slotdata[i] = (data_type)(leaf->slotdata[i + 1]);
            }
            leaf->slotuse--;
            ...
            if (slot == leaf->slotuse)
            {
                if (parent && parentslot < (unsigned)parent->slotuse)
                {
        ...
        else // !curr->isleafnode()
        {
            ...
            if (slot == 0) {
                myleft = (left == NULL) ? NULL : (node*)(leftinner->childid[left->slotuse - 1]);
            ...
            if (slot == inner->slotuse) {
                myright = (right == NULL) ? NULL : (node*)(rightinner->childid[0]);
            ...
            if (result.has(btree_fixmerge))
            {
               ...
               // BTREE_ASSERT(inner->childid[slot]->slotuse == 0);
                ...
                if (inner->level == 1)
                {
                    ...
                    slot--;
                    leaf_node *child = (leaf_node*)static_cast<node*>(inner->childid[slot]);
    ...
    result_t merge_leaves(leaf_node* left, leaf_node* right, inner_node* parent)
    {
        ...
        if (left->nextleaf)
            ((leaf_node*)(left->nextleaf))->prevleaf = left;
        else
            tailleaf = left;
    ...
    static result_t merge_inner(inner_node* left, inner_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        if (selfverify)
        {
            ...
            while(leftslot <= (unsigned)parent->slotuse && parent->childid[leftslot] != left)
                ++leftslot;
        ...
        for (unsigned int i = 0; i < (unsigned)right->slotuse; i++)
        {
            left->slotkey[left->slotuse + i] = right->slotkey[i];
    ...
    static result_t shift_left_leaf(leaf_node *left, leaf_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        // fixup parent
        if (parentslot < (unsigned int)parent->slotuse) {
    ...
    static void shift_left_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        if (selfverify)
        {
            ...
            unsigned int leftslot = 0;
            while(leftslot <= (unsigned int)parent->slotuse && parent->childid[leftslot] != left)
                ++leftslot;
...
private:
            ...
            for(unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
             ...
             //   assert(subnode->level + 1 == inner->level);
                ...
                if (inner->level == 1 && slot < inner->slotuse)
                {
                    ...
                    // correct order
                    const leaf_node *leafa = (leaf_node *)static_cast<const node*>(inner->childid[slot]);
                    const leaf_node *leafb = (leaf_node *)static_cast<const node*>(inner->childid[slot + 1]);
                ...
                if (inner->level == 2 && slot < inner->slotuse)
                {
                    ...
                    const inner_node *parenta = (inner_node*)(static_cast< node*>(inner->childid[slot]));
                    const inner_node *parentb = (inner_node*)(static_cast< node*>(inner->childid[slot+1]));

                    const leaf_node *leafa = (leaf_node *)(static_cast<const node*>(parenta->childid[parenta->slotuse]));
                    const leaf_node *leafb = (leaf_node *)(static_cast<const node*>(parentb->childid[0]));
    ...
    void verify_leaflinks() const
    {
        ...
        while(n)
        {
            ...
            for(unsigned short slot = 0; (1+slot) < (n->slotuse); ++slot)
            {
            ...
            if (n->nextleaf)
            {
                assert(key_lessequal(n->slotkey[int(n->slotuse)-1], ((leaf_node*)(n->nextleaf))->slotkey[0]));

                assert(n == ((leaf_node*)(n->nextleaf))->prevleaf);
            }
            else
            {
                assert(tailleaf == n);
...
private:
    ...
    struct dump_header
    {
	    ...
	    signature[8] = 'e'; signature[9] = 0; signature[10] = 0; signature[11] = 0;
            ...
            leafslots = leafslotmax; //btree_self::leafslotmax;
            innerslots = innerslotmax; // btree_self::innerslotmax;
