...
get_options (int argc, char **argv) {
  ...
  while ((c = getopt (argc, argv, "v?hRs:e:A:a:B:b:C:c:D:d:Vo:r:w:z:")) != -1)
    switch (c) {
    case 'v':
      printf ("%s version %d\n", NAME, VERSION);
      exit (EXIT_SUCCESS);
    ...
    case '?':
      printf ("Options:\n"
	      ...
	      "  w   : Wrap Implementation to use: \n"
	      , default_SCALE, default_edgefactor,
	      A_PARAM, B_PARAM, C_PARAM,
	      (1.0 - (A_PARAM + B_PARAM + C_PARAM))
	      );
      printWrapImplTypes(stdout);
      printf(
    	  "  z   : Wrap options for implementation %d are: ", getWrapImplType());
      printWrapImplOptions(stdout);
    ...
    case 'w':
    	errno = 0;
    	int wrap = strtol (optarg, NULL, 10);
    	if (errno) {
    	  fprintf (stderr, "Error parsing wrap implementation type %s\n", optarg);
    		err = -1;
    	}
    	setWrapImpl(wrap);
    	break;
    case 'z':
    	errno = 0;
    	int wrapOpt = strtol (optarg, NULL, 10);
    	if (errno) {
    		fprintf (stderr, "Error parsing wrap implementation option %s\n", optarg);
    		err = -1;
    	}
    	setWrapImplOptions(wrapOpt);
    	break;
    case 'V':
      VERBOSE = 1;
      break;
