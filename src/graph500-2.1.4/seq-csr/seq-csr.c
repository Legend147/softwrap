...
#define XOFFR(k) WRAPLOAD64(XOFF(k))
#define XENDOFF(k) (xoff[1+2*(k)])
#define XENDOFFR(k) WRAPLOAD64(XENDOFF(k))
...
setup_deg_off (const struct packed_edge * restrict IJ, int64_t nedge)
{
  ...
  for (k = 0; k < 2*nv+2; ++k)
    WRAPSTORE64(xoff[k], 0);
  for (k = 0; k < nedge; ++k) {
    int64_t i = get_v0_from_edge(&IJ[k]);
    int64_t j = get_v1_from_edge(&IJ[k]);
    if (i != j) { /* Skip self-edges. */
      if (i >= 0) WRAP_PP64(XOFF(i));
      if (j >= 0) WRAP_PP64(XOFF(j));
    }
  }
  accum = 0;
  for (k = 0; k < nv; ++k) {
    ...
    WRAPSTORE64(XOFF(k), accum);
    accum += tmp;
  }
  WRAPSTORE64(XOFF(nv), accum);
  for (k = 0; k < nv; ++k)
    WRAPSTORE64(XENDOFF(k), XOFFR(k));
...
{
	WRAPOPEN;
  const int64_t ia = WRAPLOAD64(*(const int64_t*)a);
  const int64_t ib = WRAPLOAD64(*(const int64_t*)b);
  WRAPCLOSE;
  if (ia < ib) return -1;
  if (ia > ib) return 1;
  return 0;
...
pack_vtx_edges (const int64_t i)
{
  ...
  qsort (&xadj[XOFFR(i)], XENDOFFR(i)-XOFFR(i), sizeof(*xadj), i64cmp);
  kcur = XOFF(i);
