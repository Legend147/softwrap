...
run_bfs (void)
{
	...
	if (!rootname)
	{
			...
			OMP("omp parallel") {
				...
				MTA("mta assert nodep") OMP("omp for")
							for (k = 0; k < nedge; ++k) {
								...
								if (i != j)
								{
									WRAPSTORE32(has_adj[i], one);
									WRAPSTORE32(has_adj[j], one);
								}
							}
			}
			WRAPCLOSE;
		...
		while (m < NBFS && t < nvtx_scale) {
			...
			if (!WRAPLOAD32(has_adj[t]) || (nvtx_scale - t)*R > NBFS - m) ++t;
			else bfs_root[m++] = t++;
			WRAPCLOSE;
		}
		if (t >= nvtx_scale && m < NBFS) {
			if (m > 0) {
				fprintf (stdout, "Cannot find %d sample roots of non-self degree > 0, using %d.\n",
						NBFS, m);
				NBFS = m;
			} else {
...
#define NSTAT 9
void PRINT_STATS(char *lbl, int israte, double *stats)
{
		...
		const int NBFS, const double *bfs_time, const int64_t *bfs_nedge)
{
	...
	tm = calloc (NBFS, sizeof (*tm));
	stats = calloc (NSTAT, sizeof (*stats));
	if (!tm || !stats) {
	...
	printf ("gentime: \t%d\n", (int)(generation_time*1000000));
	printf ("construction_time: \t%20.17e\n", construction_time);
	printf ("contime: \t%d\n", (int)(construction_time*1000000));
	printf ("nbfs: %d\n", NBFS);

	memcpy (tm, bfs_time, NBFS*sizeof(tm[0]));
	statistics (stats, tm, NBFS);
	PRINT_STATS("time", 0, stats);

	for (k = 0; k < NBFS; ++k)
		tm[k] = bfs_nedge[k];
	statistics (stats, tm, NBFS);
	PRINT_STATS("nedge", 0, stats);

	for (k = 0; k < NBFS; ++k)
		tm[k] = bfs_nedge[k] / bfs_time[k];
	statistics (stats, tm, NBFS);
	PRINT_STATS("TEPS", 1, stats);
	free(stats);
	free(tm);
}
